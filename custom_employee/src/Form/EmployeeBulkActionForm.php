...
class EmployeeBulkActionForm extends ConfirmFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $action = NULL) {
    $this->action = $action;
    $session_employee = $this->session->get('custom_employee');
    if ($this->records = $session_employee['selected_items']) {
      ...
      \Drupal::messenger()->addMessage(t('No employee record to process.'), 'error');
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $batch = [
      ...
      'operations' => [
        [
          'Drupal\custom_employee\Form\EmployeeBulkActionForm::performBatchAction',
          [$this->records, $this->action],
        ],
      ],
      'finished' => 'Drupal\custom_employee\Form\EmployeeBulkActionForm::onFinishBatchCallback',
    ];
    batch_set($batch);
    $this->session->remove('custom_employee');
